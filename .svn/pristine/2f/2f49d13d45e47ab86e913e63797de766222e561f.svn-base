<template>
  <div>
    <el-select v-model="intentForm.form.jobCodeList" :popper-append-to-body="true" @visible-change="showBox" @change="change"  :placeholder="placeholder" ref="select" multiple>
      <el-option v-for="item in options" :key="item.id" :label="item.name" :value="item.id">
      </el-option>
    </el-select>
    <el-dialog ref="modal" @close="close" @closed="closed" @open="open" title="请选择职位" class="selector_box" :visible.sync="showSelectorBox" width="770px" :modal-append-to-body="true" @mousedown="mousedown($event)">
...
import CheckAreaList from './check-pos-list.vue'
 import { getJobTypeList } from '../../api/dictionary'
import { getSiteByIds, getJsonByIds } from '../../common/common'
...
export default class PosSelector extends Vue {
  public intentForm = intentForm
  public showSelectorBox: boolean = false
  public placeholder: string = '请选择期望职位'
  ...
  created() {
    this.setOptions()
    this.value = intentForm.form.jobCodeList
  }
  ...
  public async setOptions() {
    if (this.type === 'area') {
      ...
      this.options = getJsonByIds(this.value.concat(), jobTypeList)
    }
  }
  public showBox() {
  ...
  public close() {
    ...
    this.drag = true
    this.intentForm.form.jobCodeList = this.value
  }
  public closed() {
    this.translateX = 0
    this.translateY = 0
